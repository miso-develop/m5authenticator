...
const COPY: Record<UiLanguage, HelpDiagramsCopy> = {
  en: {
    setup: {
      ...
      dailyTitle: "Daily use after boot, reconnect, or page reload",
      ...
      dailySteps: [
        { domain: "device", text: "After an actual reboot or power loss, the provisioned Device starts LOCKED. A page reload or transport reconnect alone may preserve UNLOCKED on the same running Device." },
        { domain: "browser", text: "If the Device is LOCKED, choose Unlock from Provisioner. If it is still UNLOCKED, reconnect without creating an implicit Lock." },
    ...
    trust: {
      ...
      recovery: {
        ...
        items: [
          ...
          "The Recovery Passphrase is also not stored on the Device; a Device Factory Reset cannot erase separately held Passphrase knowledge.",
        ],
      },
  ...
  ja: {
    setup: {
      ...
      dailyTitle: "起動・再接続・ページ再読み込み後の日常利用",
      browserLabel: "ブラウザ",
      deviceLabel: "Device",
      ...
      dailySteps: [
        { domain: "device", text: "実際の再起動または電源断後はProvisioning済みDeviceがLOCKEDから開始します。同じDevice runtimeでのページ再読み込みやtransport再接続だけならUNLOCKEDを維持する場合があります。" },
        { domain: "browser", text: "DeviceがLOCKEDならProvisionerから「ロック解除」を選びます。UNLOCKEDを維持している場合は暗黙のLOCKを行わず再接続します。" },
    ...
    trust: {
      ...
      recovery: {
        ...
        items: [
          ...
          "Recovery PassphraseもDeviceには保存されないため、Device Factory Resetで別に保持しているPassphraseの知識が消えることはありません。",
        ],
      },
      boundaryLabel: "Security boundary",
