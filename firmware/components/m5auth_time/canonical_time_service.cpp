...
SyncResult TimeService::sync_ntp_once() {
    ...
    if (status == vault_runtime::Status::kLocked ||
        status == vault_runtime::Status::kInvalidState) {
        wipe_string(&ssid);
        wipe_string(&password);
    ...
    const SyncResult outcome = connect_and_sync(ssid, password);
    wipe_string(&ssid);
    wipe_string(&password);
    return outcome;
}
...
SyncResult TimeService::sync_from_usb(std::uint64_t unix_seconds) {
    ...
    const auto apply_sync = [&]() {
        if (!vault_runtime_.unlocked()) return SyncResult::kLocked;
...
SyncResult TimeService::resync_if_due() {
    ...
    if (current.readiness == Readiness::kNotSynced) {
        ...
        // A LOCKED poll must not consume the initial-sync retry budget. The
        // first periodic poll after cold-boot unlock is immediately eligible.
