...
bool read_u32(cJSON* item, std::uint32_t* value) {
    int parsed = 0;
    if (value == nullptr || !read_nonnegative_integer(item, &parsed) || parsed == 0) return false;
    *value = static_cast<std::uint32_t>(parsed);
    return true;
}

bool read_u64(cJSON* item, std::uint64_t* value) {
    constexpr double kMaxSafeJsonInteger = 9'007'199'254'740'991.0;
    if (value == nullptr || !cJSON_IsNumber(item) || !std::isfinite(item->valuedouble) ||
        item->valuedouble < 0 || item->valuedouble > kMaxSafeJsonInteger) return false;
...
bool read_string(cJSON* object, const char* key, std::string* value) {
    ...
    if (!cJSON_IsString(item) || item->valuestring == nullptr || value == nullptr) return false;
...
void wipe_sensitive_json(cJSON* item) {
    ...
    for (cJSON* child = item->child; child != nullptr; child = child->next) {
        if (child->string != nullptr && cJSON_IsString(child) && child->valuestring != nullptr &&
            (std::strcmp(child->string, "secret") == 0 || std::strcmp(child->string, "password") == 0)) {
...
void add_time_status(cJSON* data, const time::Snapshot& status) {
    cJSON_AddStringToObject(data, "time_state", time::readiness_name(status.readiness));
    cJSON_AddStringToObject(data, "time_source", time::source_name(status.source));
    cJSON_AddBoolToObject(data, "time_resync_due", status.resync_due);
    if (status.readiness == time::Readiness::kNotSynced) {
        cJSON_AddNullToObject(data, "last_sync");
        cJSON_AddNullToObject(data, "time_age_seconds");
    } else {
        cJSON_AddNumberToObject(data, "last_sync", static_cast<double>(status.last_sync_unix_seconds));
        cJSON_AddNumberToObject(data, "time_age_seconds", status.age_seconds);
    }
}

std::string hello_response(int id, const core::DeviceMetadata& metadata, const storage::Store& store,
                           const time::TimeService& time_service) {
    cJSON* root = success_root(id);
    if (root == nullptr) return serialize(nullptr);
    ...
    if (!store.ready()) cJSON_AddStringToObject(data, "storage_status", storage::status_code(store.initialization_status()));
    add_time_status(data, time_service.status());
    return serialize(root);
}

std::string time_status_response(int id, const time::TimeService& time_service) {
    cJSON* root = success_root(id);
    if (root == nullptr) return serialize(nullptr);
    cJSON* data = cJSON_AddObjectToObject(root, "data");
    if (data == nullptr) { cJSON_Delete(root); return serialize(nullptr); }
...
std::string accounts_list_response(int id, storage::Store& store) {
    ...
    if (status != storage::Status::kOk) return storage_error_response(id, status);
    cJSON* root = success_root(id);
    if (root == nullptr) return serialize(nullptr);
    cJSON* data = cJSON_AddObjectToObject(root, "data");
    cJSON* array = data == nullptr ? nullptr : cJSON_AddArrayToObject(data, "accounts");
    if (data == nullptr || array == nullptr) { cJSON_Delete(root); return serialize(nullptr); }
    cJSON_AddNumberToObject(data, "count", accounts.size());
    for (const auto& account : accounts) {
        cJSON* item = cJSON_CreateObject();
        if (item == nullptr) { cJSON_Delete(root); return serialize(nullptr); }
...
std::string selection_response(int id, std::uint32_t selected_id) {
    ...
    cJSON* data = cJSON_AddObjectToObject(root, "data");
    if (data == nullptr) { cJSON_Delete(root); return serialize(nullptr); }
    if (selected_id == 0) cJSON_AddNullToObject(data, "id"); else cJSON_AddNumberToObject(data, "id", selected_id);
    return serialize(root);
}
...
Session::Session(const core::DeviceMetadata& metadata, storage::Store& store, time::TimeService& time_service)
    : metadata_(metadata), store_(store), time_service_(time_service) {}

Session::~Session() { clear_import(); }

void Session::clear_import() {
    for (auto& account : import_accounts_) storage::secure_clear(&account.secret);
...
std::string Session::handle_line(std::string_view line) {
    ...
    if (!read_nonnegative_integer(cJSON_GetObjectItemCaseSensitive(root, "id"), &id)) {
        delete_request(root); return error_response(0, "invalid_id");
    }
    int version = 0;
    if (!read_nonnegative_integer(cJSON_GetObjectItemCaseSensitive(root, "v"), &version) ||
        version != core::kProtocolVersion) {
        delete_request(root); return error_response(id, "unsupported_version");
    }
    ...
    if (!cJSON_IsString(op) || op->valuestring == nullptr || !cJSON_IsObject(params)) {
        delete_request(root); return error_response(id, "invalid_request");
    }
    const std::string operation(op->valuestring);
    std::string response;
    if (operation == "hello") {
        response = hello_response(id, metadata_, store_, time_service_);
    } else if (operation == "time.status") {
        response = time_status_response(id, time_service_);
    } else if (operation == "time.sync") {
        std::uint64_t unix_seconds = 0;
        if (!read_u64(cJSON_GetObjectItemCaseSensitive(params, "unix_seconds"), &unix_seconds)) {
            response = error_response(id, "invalid_request");
        } else {
            const time::SyncResult result = time_service_.sync_from_usb(unix_seconds);
            response = result == time::SyncResult::kOk
                ? time_status_response(id, time_service_)
                : error_response(id, time::sync_result_code(result));
        }
    } else if (operation == "accounts.list") {
        response = accounts_list_response(id, store_);
    } else if (operation == "import.begin") {
        clear_import(); import_active_ = true; response = import_begin_response(id);
    } else if (operation == "import.item") {
        storage::AccountDraft draft;
        const bool parsed = import_active_ && read_string(params, "issuer", &draft.issuer) &&
            read_string(params, "account", &draft.account) &&
            read_string(params, "display_name", &draft.display_name) && read_string(params, "secret", &draft.secret);
        wipe_json_string(params, "secret");
        if (!parsed) {
            storage::secure_clear(&draft.secret); response = error_response(id, "invalid_import_item");
        } else if (import_accounts_.size() >= storage::kMaxAccounts) {
            storage::secure_clear(&draft.secret); response = error_response(id, "storage_full");
        } else {
            const storage::Status status = storage::validate_account_draft(draft);
            if (status != storage::Status::kOk) {
                storage::secure_clear(&draft.secret); response = storage_error_response(id, status);
            } else {
                import_accounts_.push_back(std::move(draft)); import_validated_ = false;
                response = count_success(id, import_accounts_.size());
            }
        }
    } else if (operation == "import.validate") {
        if (!import_active_) response = error_response(id, "import_not_active");
        else {
            ...
            for (const auto& account : import_accounts_) {
                ...
                if (status != storage::Status::kOk) break;
            }
            if (status != storage::Status::kOk) response = storage_error_response(id, status);
            else { import_validated_ = true; response = count_success(id, import_accounts_.size()); }
        }
    } else if (operation == "import.commit") {
        if (!import_active_ || !import_validated_) response = error_response(id, "import_not_validated");
        else {
            const storage::Status status = store_.replace_accounts(import_accounts_);
            clear_import(); response = status == storage::Status::kOk ? empty_success(id) : storage_error_response(id, status);
        }
    } else if (operation == "import.cancel") {
        clear_import(); response = empty_success(id);
    } else if (operation == "account.rename") {
        std::uint32_t account_id = 0; std::string display_name;
        if (!read_u32(cJSON_GetObjectItemCaseSensitive(params, "id"), &account_id) ||
            !read_string(params, "display_name", &display_name)) response = error_response(id, "invalid_request");
        else {
        ...
        if (!read_u32(cJSON_GetObjectItemCaseSensitive(params, "id"), &account_id)) response = error_response(id, "invalid_request");
        else {
        ...
        if (count < 0 || static_cast<std::size_t>(count) > storage::kMaxAccounts) response = error_response(id, "invalid_request");
        else {
            std::vector<std::uint32_t> order; bool valid = true; cJSON* item = nullptr;
            cJSON_ArrayForEach(item, ids) {
                std::uint32_t account_id = 0;
                if (!read_u32(item, &account_id)) { valid = false; break; }
                order.push_back(account_id);
            }
            if (!valid) response = error_response(id, "invalid_request");
            else {
        ...
        std::uint32_t selected = 0; const storage::Status status = store_.get_last_used(&selected);
        response = status == storage::Status::kOk ? selection_response(id, selected) : storage_error_response(id, status);
    } else if (operation == "selection.set") {
        std::uint32_t selected = 0;
        if (!read_u32(cJSON_GetObjectItemCaseSensitive(params, "id"), &selected)) response = error_response(id, "invalid_request");
        else {
        ...
        const bool parsed = read_string(params, "ssid", &ssid) && read_string(params, "password", &password);
        wipe_json_string(params, "password");
        if (!parsed) { storage::secure_clear(&password); response = error_response(id, "invalid_request"); }
        else {
...
std::string message_too_large_response() { return error_response(0, "message_too_large"); }
